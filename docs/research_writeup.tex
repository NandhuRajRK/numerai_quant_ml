...
This was the best medium-scale ensemble score observed in the project. However, the MLX model itself was not the strongest standalone learner. Its value appeared to come from diversification rather than raw standalone accuracy.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item MLX-based neural models became competitive enough to improve a medium-scale ensemble result, even if they are not yet the most stable branch.
\end{enumerate}
...
The medium sample run still uses 12 validation eras, which is useful for iteration and documentation but not enough for a strong claim on its own. Stronger benchmark configurations with 60 validation eras have already been added to address this.
...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item an \textbf{MLX-inclusive branch}, which may produce higher medium-run ensemble scores but has not yet shown the same robustness under blend optimization.
\end{itemize}
...
\begin{enumerate}[leftmargin=1.5em]
  \item run the 60-validation-era benchmark configurations and replace medium-scale claims with stronger evidence;
